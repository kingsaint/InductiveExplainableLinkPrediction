...
def print_params(model):
    # /* Print the parameters and their numbers
    num_gtrans_params = 0
    num_policy_params = 0
    ...
    for name, p in model.named_parameters():
        if p.requires_grad:
            print(name, p.numel())
            if 'graph_transformer' in name:
                num_gtrans_params += p.numel()
                if 'attentions' in name:
                    num_attention_params += p.numel()
            else:
                num_policy_params += p.numel()
    print("# parameters in Graph Transformer = {}".format(num_gtrans_params))
    print("# parameters for Attention = {}".format(num_attention_params))
    print("# parameters for Policy = {}".format(num_policy_params))
...
def inference(lf):
    ...
    if args.compute_map:
        ...
        # Evaluation on test data by relation types
        to_m_rels, to_1_rels, _ = data_utils.get_relations_by_type(args.data_dir, relation_index_path)
        relation_by_types = (to_m_rels, to_1_rels)
        print('Test set evaluation by relation type (full graph)')
        src.eval.hits_and_ranks_by_relation_type(
            test_data, pred_scores, lf.kg.all_objects, relation_by_types, verbose=True)
    return eval_metrics
